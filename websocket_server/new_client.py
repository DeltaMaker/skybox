...
class SimpleWebsocketClient:
    ...
    async def subscribe(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not self.validate_subscription_response(confirmation):
                raise ConnectionError("Server did not confirm subscription")
            
        except Exception as e:
            raise ConnectionError(f"Failed to subscribe: {str(e)}")

    def validate_subscription_response(self, response: dict) -> bool:
        """Check if the response is a subscription confirmation."""
